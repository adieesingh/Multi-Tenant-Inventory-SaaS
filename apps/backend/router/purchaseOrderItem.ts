...
purchaseOrderItemRouter.post("/", middleware, async (req, res) => {
  try {
    const purchaseOrderItemPayLoad = PurchaseOrderItemSchema.safeParse(
      req.body,
    );
    ...
    const total =
      purchaseOrderItemPayLoad.data.quantity *
      purchaseOrderItemPayLoad.data.unitCost;
    await prismaClient.purchase_Order_Item
      .create({
        data: {
